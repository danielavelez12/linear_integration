...
def handle_new_task(new_task):
    ...
    response = query_openai(prompt, tools=tools, tool_choice={"type": "function", "function": {"name": "decide_action"}})
...
def main(chat_transcript):
    ...
    response = query_openai(chat_transcript, tools=tools, tool_choice={"type": "function", "function": {"name": "classify_transcript"}})
    
    try:
        classification = json.loads(response['choices'][0]['message']['tool_calls'][0]['function']['arguments'])
